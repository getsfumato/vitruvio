...
## The five habits the skills teach
...
5. Search first. `search` runs the planner over the indices and is the only read that ranks; `inspect blocks` is an
   unranked inventory and the last resort, after `query explain` and a rebuilt index have not helped.
