---
title: "Publishing and installing"
description: "OCI artifacts, credentials, and the Docker traps."
---
...
- **Exit 8, not a fast-forward.** Someone pushed since this brain was pulled. Reconcile: `dist fetch` brings their
  history without adopting it, and keeps both sides' work — see [Reconciling](/reconciliation/joining-two-histories). Never `--force`,
...
`dist push --all` skips it. See [Projects](/projects/publishing-a-project#a-brain-you-did-not-author) for why an installed brain
wants this.
...
one. See [Reconciling](/reconciliation/joining-two-histories).
