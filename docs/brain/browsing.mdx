---
title: "Browsing"
description: "Reading and querying in the terminal interface, and what its views can show."
---
...
vitruvio inspect content <DIGEST> --out notes.pdf
...
│ catalog    │ sha…  notes… you…  ✓    │ notes.pdf                │
│  discipline│                         │                          │
│ …          │ sha256… diagra… png     │ [the page, drawn]        │
└────────────┴─────────────────────────┴──────────────────────────┘
```
...
| tab | what it holds |
|---|---|
| preview | the bytes the block names, drawn if a terminal can draw them. Text is shown up to 64 KiB and 2,000 lines, and a footer says what was cut |
...
so the header names the brain and the layer that chose it (`coursework/mechanics by flag`, `demo/brain by
state`), and `i` prints the whole path, the snapshot, which modules are installed and who writes are attributed
to. The project is part of the short form because two projects each holding a `metric-a` is the ordinary case,
...
│ *  benchmarks  2       │ *  metric-a                 …          │
│    coursework  3       │    metric-b                 …          │
└────────────────────────┴────────────────────────────────────────┘
```
...
text view — an HTML block opens on the text view, and `t` shows its source — `]` and `[` turn PDF pages, `o` open in
whatever the desktop uses, `e` export into the working directory, `y` copy the block id, `n` and `b` page through a
large module, `r` re-read, `?` every binding.
...
| bytes | shown as |
|---|---|
| `text/*`, JSON, YAML, TOML | themselves, up to 64 KiB and 2,000 lines; Markdown rendered; code highlighted up to 32 KiB and plain past that; HTML opens on its extracted text |
...
With `--out` the same command writes the bytes somewhere permanent instead, replacing what is there — you named
the destination, so it means it. Every *derived* destination refuses instead: pressing `e` in the browser will not
write over a file in your working directory, and cannot write outside it either, because the name it derives comes
from the origin recorded in the block and a pulled brain's origins are somebody else's text.
...
The text caps exist because the pane lays text out on the interface's own thread, once per paint and again on
every resize, and syntax highlighting runs there too. Past 64 KiB or 2,000 lines the footer reads
`showing 64.0 KiB of 3.1 MiB` followed by the keys that act on the block, so where the preview ends is never
mistaken for where the file ends. `inspect content` shows up to 512 KiB of the same text, and `--out FILE` writes
the whole file.
...
- **`t`** — the normalized text view, if the block has one. Real text, reflowable and searchable. A PDF gets one
  when `pdf-text` could run — the `[vision]` extra is installed, or `--normalize-with pdf-text` was named; a
  `source register file.pdf` without either stores the bytes and extracts nothing. An HTML block opens on this
  view already, since its source is the wrapper and its text is the content; there `t` goes the other way, to
  the tags.
...
[ADR-0012](https://github.com/getsfumato/vitruvio/blob/main/adr/0012-the-terminal-interface.md).
