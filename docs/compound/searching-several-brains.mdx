---
title: "Compound: one question, several brains"
sidebarTitle: "Searching several brains"
description: "One question across several brains of one project: why the result is grouped by default, what --fuse does, and what a block two brains hold means."
---

```bash
vitruvio compound search "selection bias" --brains metric-a --brains metric-b
vitruvio compound search "selection bias" --brains metric-a,metric-b --fuse
vitruvio compound search "selection bias" --all
vitruvio compound explain "selection bias" --all
```

A project is several brains under one configuration -- a subject per brain, a client per brain, a metric per brain
([chapter 13](/projects)) -- and every command so far addresses one of them. A **compound** asks two or more the
...
```
error: this project has no brain called './brains/metric-a'
hint: a compound takes brain names from this project only; known: metric-a, metric-b, metric-c
```
...
([chapter 6](/under-the-hood/the-planner)), consults its own indices, and verifies against its own roots. What a compound adds
...
verified against  canonical sha256:151df9b233  semantic sha256:7e77a9fd08
 ...
 1    1.00   semantic   sha256:4ec8c0ba55   Selection bias
 2    0.98   semantic   sha256:dc7f65edbf   Sampling
...
This is the default because of something [chapter 5](/brain/searching) says about a single bundle: a score is
...
```
 ...
 1    1.00   metric-a#1  metric-b#1   semantic   sha256:4ec8c0ba55   Selection bias
 2    0.61   metric-a#2               semantic   sha256:dc7f65edbf   Sampling
```

Reciprocal-rank fusion across brains -- the same rule the planner uses across generators inside one brain
([ADR-0005](https://github.com/getsfumato/vitruvio/blob/main/adr/0005-statistics-and-the-cost-model.md)). Each brain that returned a block contributes
...
from both and rises. `brains` on the match says so: `metric-a#1  metric-b#1` is two subjects agreeing, and that
...
## Counting across brains with SQL

`compound search` asks each brain separately and composes the answers. A count or a join across brains is one
question about all of them together, so `compound sql` loads every brain's tables into one database and runs the
query once:

```bash
vitruvio compound sql "SELECT brain, count(*) FROM semantic WHERE kind = 'fact' GROUP BY brain" --all
vitruvio compound sql 'SELECT a.label FROM algebra.semantic a JOIN "analisis-ii".semantic b ON b.id = a.id' \
  --brains algebra,analisis-ii
```

- A bare table such as `semantic` covers every brain consulted and gains a leading `brain` column. A qualified
  one, such as `algebra.semantic`, reads one brain. Quote a brain name that contains a dash.
- Each brain hides superseded blocks according to its own provenance, uses its own table cache, and scores
  `about()` with its own vectors.
- Blocks are content-addressed, so a block held by two brains is one row per brain. Use `count(DISTINCT id)` to
  count it once.
- Every key in `verified_against`, `hidden` and `not_installed` is written `brain.module`. `hidden` is counted
  per brain even when the query reads a bare table.
- `about()` is scored only by the brains the query reads. `algebra.semantic` is scored by algebra's model alone. `skipped` names the
  brains that were declared but have no layout on this machine.
