...
brain that fails for a real reason does not stop the others, and the command exits non-zero at the end — carrying
the per-brain results with it, so `--json` on a half-published project still tells you which ones went.
