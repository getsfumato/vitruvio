...
1. **A validity rule, never costed.** With two or more scoring generators available, a plan using fewer than two is
   ...
   an exhaustive scan is a *no*-authority plan, not a single-authority one.) The scan earns its recall of 1.0 by
   reading what the indices read: a canonical block's normalized view, through the module store, so every generator
   observes one text. A block registered with no view matches on its media type alone, for the scan exactly as for the
   indices, and `explain` says how many such blocks a canonical scan is walking past.
